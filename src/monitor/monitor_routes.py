#!/usr/bin/env python3
"""
SellAI 商机监控模块 v3.2.4
===========================
重建商机监控功能，支持主动爬取和毛利筛选
...
class MonitorService:
    ...
    def scan_opportunities(
        ...
        Returns:
            扫描结果字典
        """
        logger.info(f"商机扫描启动 - 门槛:{threshold}%, 最大结果:{max_results}, 平台:{platforms}")
        ...
        try:
            ...
            logger.info(f"商机扫描完成 - 发现{len(opportunities)}个商机")
            return result
...
async def monitor_active_handler(request: dict) -> dict:
    ...
    return monitor_service.scan_opportunities(threshold, max_results, platforms)
