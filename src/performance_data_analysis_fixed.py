...
class PerformanceDataAnalyzer:
    ...
    def analyze_communication_efficiency(self):
        ...
        if len(df) > 0:
            ...
            for op_type in df['operation_type'].unique():
                op_data = df[df['operation_type'] == op_type]
                operation_stats[op_type] = {
                    'message_size_mean': float(op_data['message_size_bytes'].mean()),
                    'message_size_min': int(op_data['message_size_bytes'].min()),
                    'message_size_max': int(op_data['message_size_bytes'].max()),
                    'processing_time_mean': float(op_data['processing_time_ms'].mean()),
                    'processing_time_min': int(op_data['processing_time_ms'].min()),
                    'processing_time_max': int(op_data['processing_time_ms'].max()),
                    'count': int(len(op_data))
                }
            
            print("\n按操作类型统计:")
            for op_type, stats in operation_stats.items():
                print(f"  {op_type}:")
                print(f"    消息大小: 平均{stats['message_size_mean']:.1f}字节, "
                      f"范围{stats['message_size_min']}-{stats['message_size_max']}字节")
                print(f"    处理时间: 平均{stats['processing_time_mean']:.1f}ms, "
                      f"范围{stats['processing_time_min']}-{stats['processing_time_max']}ms")
            ...
            processing_time_stats = {
                'mean': float(df['processing_time_ms'].mean()),
                'std': float(df['processing_time_ms'].std()),
                'min': float(df['processing_time_ms'].min()),
                'max': float(df['processing_time_ms'].max()),
                'p50': float(df['processing_time_ms'].quantile(0.5)),
                'p95': float(df['processing_time_ms'].quantile(0.95))
            }
            
            self.analysis_results['communication_metrics'] = {
                'total_records': int(len(df)),
                'avg_processing_time_ms': float(avg_processing_time),
                'avg_message_size_bytes': float(avg_message_size),
    ...
    def analyze_avatar_capabilities(self):
        ...
        for _, row in df.iterrows():
            try:
                ...
                scores['success_rate'] = float(row['success_rate'])
                scores['total_tasks'] = int(row['total_tasks_completed'])
                scores['current_load'] = int(row['current_load'])
        ...
        if capability_data:
            ...
            # 分析负载分布
            load_distribution = {}
            for load in sorted(df['current_load'].unique()):
                count = int((df['current_load'] == load).sum())
                load_distribution[int(load)] = count
            ...
            capability_gaps = {}
            for capability, score in min_capabilities.items():
                if score < 0.7:
                    capability_gaps[capability] = float(score)
    ...
    def analyze_cost_efficiency(self):
        ...
        if len(df) > 0:
            # 按分身统计成本
            cost_by_avatar = {}
            for avatar in df['avatar_id'].unique():
                avatar_data = df[df['avatar_id'] == avatar]
                total_cost = float(avatar_data['total_cost'].sum())
                total_amount = float(avatar_data['amount'].sum())
                cost_by_avatar[avatar] = {
                    'total_cost': total_cost,
                    'total_amount': total_amount
                }
            
            print("\n按分身统计成本（降序排列）:")
            sorted_avatars = sorted(cost_by_avatar.items(), key=lambda x: x[1]['total_cost'], reverse=True)
            for avatar, data in sorted_avatars:
                print(f"  {avatar}: 总成本${data['total_cost']:.4f}, 总量{data['total_amount']:.1f}")
            
            # 按成本类型统计
            cost_by_type = {}
            for cost_type in df['cost_type'].unique():
                type_data = df[df['cost_type'] == cost_type]
                total_cost = float(type_data['total_cost'].sum())
                total_amount = float(type_data['amount'].sum())
            ...
            for cost_type, data in cost_by_type.items():
                print(f"  {cost_type}: 总成本${data['total_cost']:.4f}, 总量{data['total_amount']:.1f}")
            
            # 计算平均成本和效率指标
            total_cost = float(df['total_cost'].sum())
            avg_cost_per_avatar = total_cost / len(df['avatar_id'].unique()) if len(df['avatar_id'].unique()) > 0 else 0
            ...
            # 分析成本构成
            cost_breakdown = {}
            for cost_type in ['tokens', 'api_calls', 'workflow_executions']:
                if cost_type in df['cost_type'].values:
                    cost_data = df[df['cost_type'] == cost_type]
                    cost_breakdown[cost_type] = float(cost_data['total_cost'].sum())
                else:
    ...
    def analyze_system_health(self):
        ...
        if len(df) > 0:
            ...
            # 分析成功率
            avg_success_rate = float(df['task_success_rate'].mean())
            success_rate_stats = {
                'min': float(df['task_success_rate'].min()),
                'max': float(df['task_success_rate'].max()),
                'mean': float(df['task_success_rate'].mean()),
                'std': float(df['task_success_rate'].std()),
                'p25': float(df['task_success_rate'].quantile(0.25)),
                'p50': float(df['task_success_rate'].quantile(0.5)),
                'p75': float(df['task_success_rate'].quantile(0.75))
            }
            ...
            for _, node in df.iterrows():
                if node['status'] not in ['healthy', 'unknown']:
                    problem_nodes.append({
                        'node_id': node['node_id'],
                        'status': node['status'],
                        'error_count': int(node['error_count'])
                    })
